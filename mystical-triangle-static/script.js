...
function validateDates(dates) {
    ...
    // Accept DD/MM/YYYY where 00 / 0000 means unknown part
    const dateRegex = /^\d{2}\/\d{2}\/\d{4}$/;

    for (let i = 0; i < dates.length; i++) {
        const dateStr = dates[i];
        if (!dateStr || dateStr.trim() === "") continue; // dates 2 & 3 can be empty

        if (!dateRegex.test(dateStr))
            return `Date ${i + 1} must be DD/MM/YYYY (use 00 or 0000 for unknown parts, e.g. 00/08/1990).`;

        const [d, m] = dateStr.split('/').map(Number);
        if (d > 31) return `Date ${i + 1}: day must be 00–31.`;
        if (m > 12) return `Date ${i + 1}: month must be 00–12.`;
    }
    return null;
}
